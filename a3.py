...
class CommandInterface:
    ...
    def match_pattern(self, pattern, board_segment):
        ...
        """
        for p, b in zip(pattern, board_segment):
            if p == '.':
                continue
            if p == 'X' and b is not None:
                return False
            if p != 'X' and p != b:
                return False
        return True
    ...
    def calculate_weight(self, x, y, num):
        row_pattern_weight = self.default_weight
        col_pattern_weight = self.default_weight
        
        # Check for matching row patterns
        row_segment = [self.board[y][i] if 0 <= i < len(self.board[0]) else 'X' for i in range(x-2, x+3)]
        for pattern, move, weight in self.patterns:
            if move == num and self.match_pattern(pattern, row_segment):
                row_pattern_weight = weight
                break
        
        # Check for matching column patterns
        col_segment = [self.board[i][x] if 0 <= i < len(self.board) else 'X' for i in range(y-2, y+3)]
        for pattern, move, weight in self.patterns:
            if move == num and self.match_pattern(pattern, col_segment):
                col_pattern_weight = weight
                break

        # The total weight is the sum of row and column weights
        return row_pattern_weight + col_pattern_weight
    ...
    def policy_moves(self, args):
        ...
        total_weight = sum(weight for move, weight in move_weights)

        # Compute and print the probabilities
        move_probs = [(move, round(weight / total_weight, 3)) for move, weight in move_weights]
        
        move_probs.sort(key=lambda x: (int(x[0][0]), int(x[0][1]), int(x[0][2])))  # Sort by x, y, num
        
        result = []
        for move, prob in move_probs:
            result.extend(move + [str(prob)])
