...
class CommandInterface:
    ...
    def loadpatterns(self, args):
        if len(args) != 1:
            print("Usage: loadpatterns filename")
            return False
        ...
        try:
            with open(filename, 'r') as file:
                for line in file:
                    ...
                    if len(parts) != 2:
                        print(f"Invalid pattern line: {line}")
                        continue
                    pattern, weight = parts[0], float(parts[1])
    ...
    def policy_moves(self, args):
        if not hasattr(self, 'pattern_weights'):
            print("No patterns loaded. Please load patterns before using policy_moves.")
            return False
        ...
        for move in moves:
            pattern = self.extract_pattern(move)
            weight = self.pattern_weights.get(pattern, 1)  # Default weight is 1 if pattern not found
        ...
        self.play(selected_move)
        print(" ".join(selected_move))
        return True
